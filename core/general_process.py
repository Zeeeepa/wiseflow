        ...
        try:
            ...
            if not author or author.lower() == 'na' or not publish_date or publish_date.lower() == 'na':
                try:
                    ...
                    wiseflow_logger.error(f"Error extracting author/date from {url}: {e}")
            ...
            # Normalize publish date format
            publish_date = extract_and_convert_dates(publish_date)

            # Process extracted information
            try:
                await info_process(url, title, author, publish_date, contents, link_dict, focus_id, get_info_prompts)
                wiseflow_logger.info(f"Successfully processed {url}")
            except Exception as e:
                wiseflow_logger.error(f"Error in info_process for {url}: {e}")
                # Save raw data to file for debugging
                timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
                debug_file = os.path.join(project_dir, f'{timestamp}_failed_process_{focus_id}.json')
                try:
                    with open(debug_file, 'w', encoding='utf-8') as f:
                        json.dump({
                            ...
                            'author': author,
                            'publish_date': publish_date,
                            'content_length': len(str(contents)),
                            'error': str(e)
                        }, f, ensure_ascii=False, indent=4)
                    wiseflow_logger.info(f"Saved debug info to {debug_file}")
                except Exception as write_err:
                    wiseflow_logger.error(f"Failed to write debug file: {write_err}")
                
        except Exception as e:
            wiseflow_logger.error(f"Unhandled error processing URL {url}: {e}")
            # Log full traceback for debugging
            import traceback
            wiseflow_logger.error(f"Traceback: {traceback.format_exc()}")

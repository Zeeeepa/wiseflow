...
class AsyncWebCrawler:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._connection_pool_size = self.browser_config.max_connections or 10
        ...
        try:
            while True:
                ...
                if avg_memory > self.memory_warning_percent:
                    ...
                    # If memory is still high after garbage collection, reduce connection pool size
                    if psutil.virtual_memory().percent > self.memory_warning_percent:
                        new_pool_size = max(1, self._connection_pool_size // 2)
                        self.logger.warning(f"Reducing connection pool size from {self._connection_pool_size} to {new_pool_size}")
                        self._connection_pool_size = new_pool_size
                        # Create a new semaphore with reduced size
                        self._connection_semaphore = asyncio.Semaphore(self._connection_pool_size)
    ...
    async def arun(
        ...
        **kwargs,
        ...
        async with self._lock or self.nullcontext():
            ...
            # Use connection pool semaphore to limit concurrent connections
